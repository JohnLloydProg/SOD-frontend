...
export class StackCarousel implements OnInit, OnDestroy{
  ...
  getCardClasses(index: number, activeIndex: number): string {
    ...
    if (offset === 0) {
      return 'z-40 opacity-100 scale-100 translate-y-0 rotate-0 cursor-auto';
    }


    if (offset === 1) {
      return 'z-30 opacity-95 scale-95 translate-y-4 rotate-5';
    }


    if (offset === 2) {
      return 'z-20 opacity-90 scale-90 -translate-y-1/5 -rotate-10';
    }

    if (offset === 3) {
      return 'z-10 opacity-85 scale-90 -translate-y-1/4 rotate-5';
    }
